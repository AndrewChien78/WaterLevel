SubsetByMonth <- function(timeseries.df, set.month) {
  # Subsets a times series dataframe with a timestamp column called Date.and.Time into 
  # a month 
  # 
  # Args:
  #   timeseries.df: Time series dataframe that contains a date time column in the class 
  #   of factor or character
...
SubsetByDay <- function(timeseries.df, set.day) {
  # Subsets a times series dataframe with a timestamp column called Date.and.Time into 
  # a month 
  # 
  # Args:
  #   timeseries.df: Time series dataframe that contains a date time column in the class  
  #   of factor or character
  #   set.day: a string that is the format "YYYY-MM-DD"
  #
  #   Returns:
  #   one.day: A dataframe that has been subset-ed to a day.  
  # Date: 18 Febuary 2019
  # Author: Andrew Chien
  # TODO(AndrewC): Need to add error handling for arguments
  ...
  #adds the string "00:00:00" to complete the date time as first second of the day
  startdatetime <- paste(set.day, " 00:00:00", sep="")
  
  #calculates the last second of set.day
  enddatetime <- (seq(as.Date(startdatetime), length=2, by="day") - 1)
  enddatetime <- (enddatetime[-1] + 1)
  ...
  #subsets timeseries.df to the set startdatetime and enddatetime range
  one.day <- subset(timeseries.df,timeseries.df$Date.and.Time  >= startdatetime 
                      & timeseries.df$Date.and.Time < enddatetime)
